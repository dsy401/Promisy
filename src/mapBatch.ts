...
export async function mapBatch<T, S>(items: T[], cb: (item: T, i: number) => S | Promise<S>, batchSize: number): Promise<S[][]>
export async function mapBatch<T, S>(items: T[], cb: (item: T, i: number) => S | Promise<S>): Promise<S[][]>

export async function mapBatch<T, S>(items: T[], cb: (item: T, i: number) => S | Promise<S>, batchSize: number = 10): Promise<S[][]> {
    const batches: T[][] = []
